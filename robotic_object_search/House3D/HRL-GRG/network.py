    ...
    def __init__(self,
        ...
        with tf.variable_scope('highlevel'):
            with tf.variable_scope(scope):
                self.visions = tf.placeholder(shape=[None, history_steps * window_size * window_size, 1],
                                              dtype=tf.float32)
                ...
                # self.planning_results = tf.placeholder(shape=[None, num_labels], dtype=tf.float32)

                # object_relations = tf.get_variable(name='relations',
                #                                    shape=[num_labels, num_labels],
                #                                    initializer=tf.contrib.layers.xavier_initializer(),
                #                                    trainable=True)
                # target_relations = tf.matmul(self.targets, object_relations)

                # target_relations_expanded = tf.tile(tf.expand_dims(self.planning_results, 1),
                #                                     [1, history_steps*window_size*window_size, 1])
                #
                # related_visions = self.visions * target_relations_expanded
                #
                # related_visions = tf.reshape(related_visions, [-1, num_labels, history_steps*window_size*window_size])
