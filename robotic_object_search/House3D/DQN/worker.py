...
class Worker():
    ...
    def _train_highlevel(self,
                         sess):
        # replay_buffer:
        # [0:vision_feature, 1:depth_feature, 2:plannig_results, 3:target_input, 4:action, 5:reward,
        ...
        with sess.as_default():
            ...
            next_q_prime = sess.run(self.highlevel_network_target.q_values,
                                    feed_dict={self.highlevel_network_target.visions:np.stack(batch[:, 6]),
                                               self.highlevel_network_target.depths:np.stack(batch[:, 7]),
                                               self.highlevel_network_target.targets:np.stack(batch[:, 3])})
            next_q = sess.run(self.highlevel_network_main.q_values,
                              feed_dict={self.highlevel_network_main.visions:np.stack(batch[:, 6]),
                                         self.highlevel_network_main.depths:np.stack(batch[:, 7]),
                                         self.highlevel_network_main.targets:np.stack(batch[:, 3])})
            target_q_values = batch[:, 5] + (1 - batch[:, 9]) * FLAGS.gamma * \
    ...
    def _run_training_episode(self,
        ...
        for _ in range(FLAGS.max_episode_steps):
            ...
            if not testing and highlevel_steps != 0:
                highlevel_sample += [extrinsic_reward,
                                     vision_feature,
                                     depth_feature,
                                     valid_targets,
                                     done]
                self.highlevel_replay_buffer.add(np.reshape(np.array(highlevel_sample), [1, -1]))
            ...
            qvalues = sess.run(self.highlevel_network_main.q_values,
                               feed_dict={self.highlevel_network_main.visions: [vision_feature],
                                          self.highlevel_network_main.depths: [depth_feature],
                                          self.highlevel_network_main.targets: [target_input]})[0]
            ...
            highlevel_sample = [vision_feature,
                                depth_feature,
                                planning_results[1],
                                target_input,
                                action]
        ...
        if testing:
            ...
            return disc_cumu_rewards, action_steps, min_step, done, left_step, states_buffer, subtargets_buffer, actions_buffer
        ...
        return disc_cumu_rewards, action_steps, min_step, done, l, pl, vl, el, ql, \
