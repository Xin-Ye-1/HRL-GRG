...
class Worker():
    ...
    def _train_highlevel(self,
                         sess):
        # replay_buffer:
        # [0:vision_feature, 1:depth_feature, 2:target_input, 3:subtarget, 4:reward,
        #  5:next_vision_feature, 6:next_depth_feature, 7:next_valid_targets, 8:done]
        with sess.as_default():
            batch = self.highlevel_replay_buffer.sample(FLAGS.batch_size)
            next_q_prime = sess.run(self.highlevel_network_target.q_values,
                                    feed_dict={self.highlevel_network_target.visions:np.stack(batch[:, 5]),
                                               self.highlevel_network_target.depths:np.stack(batch[:, 6]),
                                               self.highlevel_network_target.targets:np.stack(batch[:, 2])})
            next_q = sess.run(self.highlevel_network_main.q_values,
                              feed_dict={self.highlevel_network_main.visions:np.stack(batch[:, 5]),
                                         self.highlevel_network_main.depths:np.stack(batch[:, 6]),
                                         self.highlevel_network_main.targets:np.stack(batch[:, 2])})
            next_valid_targets = batch[:, 7]
            subtarget_id = [next_valid_targets[i][np.argmax(next_q[i][next_valid_targets[i]])]
                            for i in range(FLAGS.batch_size)]
            target_q_values = batch[:, 4] + (1 - batch[:, 8]) * FLAGS.gamma * \
            ...
            qvalue_loss, _ = sess.run([self.highlevel_network_main.qvalue_loss,
                                      ...
                                      feed_dict={self.highlevel_network_main.visions:np.stack(batch[:, 0]),
                                                 ...
                                                 self.highlevel_network_main.targets:np.stack(batch[:, 2]),
                                                 self.highlevel_network_main.chosen_objects:batch[:, 3],
        ...
        for _ in range(FLAGS.max_episode_steps):
            ...
            if highlevel_steps == 0 or sub_done or termination or lowlevel_steps == FLAGS.max_lowlevel_episode_steps:
                ...
                if np.random.rand() < epsilon:
                    ...
                    subtarget_id = valid_targets[np.argmax(qvalues[valid_targets])]
