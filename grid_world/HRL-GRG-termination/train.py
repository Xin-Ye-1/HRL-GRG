...
flags.DEFINE_integer('highlevel_num_channels', 1, 'The number of vision channels.')
flags.DEFINE_integer('lowlevel_num_channels', 2, 'The number of vision channels.')
...
flags.DEFINE_string('pretrained_model_path', '../DQN/result_for_pretrain/model', 'The path to load pretrained model from.')
...
def set_up():
    ...
    with graph.as_default():
        ...
        highlevel_network_main = Highlevel_Network(window_size=FLAGS.window_size,
                                                   channel_size=FLAGS.highlevel_num_channels,
                                                   num_goals=FLAGS.num_goals+1,
        ...
        lowlevel_network_main = Lowlevel_Network(window_size=FLAGS.window_size,
                                                 channel_size=FLAGS.lowlevel_num_channels,
                                                 num_goals=FLAGS.num_goals+1,
                                                 num_actions=FLAGS.num_actions,
                                                 history_steps=FLAGS.history_steps,
                                                 scope='global/main')
        ...
        for i in range(FLAGS.num_threads):
            ...
            relation_graph = Dirichlet_Multimodel_Graph(num_nodes=FLAGS.num_goals+1,
                                                        num_categories=FLAGS.max_lowlevel_episode_steps+1,
                                                        gamma=FLAGS.gamma)
            graph_params = np.zeros((FLAGS.num_goals+1, FLAGS.num_goals+1, FLAGS.max_lowlevel_episode_steps + 1))
            graph_params[:, :, -1] = 1
