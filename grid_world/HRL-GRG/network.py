    ...
    def __init__(self,
        ...
        with tf.variable_scope('highlevel'):
            with tf.variable_scope(scope):
                self.state = tf.placeholder(shape=[None, window_size, window_size, channel_size], dtype=tf.float32)
                self.goal = tf.placeholder(shape=[None, window_size, window_size, 1], dtype=tf.float32)
                # self.planning_results = tf.placeholder(shape=[None, num_goals], dtype=tf.float32)
                ...
                result = tf.concat((self.state, self.goal), axis=-1)
                conv_layers = [(50, [3, 3]),
                               (100, [3, 3])]
                pool_layers = [[2, 2],
                               [2, 2]]
                ...
                # flatten = tf.concat([flatten, self.planning_results], 1)
                ...
                qvalues = slim.fully_connected(inputs=hidden_embed,
                                               ...
                                               biases_initializer=tf.zeros_initializer(),
                                               scope='qvalue')
                self.qvalues = qvalues#*self.planning_results

                # training
                if scope != 'global':
                    self.chosen_goals = tf.placeholder(shape=[None], dtype=tf.int32)
                    self.target_q_values = tf.placeholder(shape=[None], dtype=tf.float32)
                    self.lr = tf.placeholder(dtype=tf.float32)

                    goals_onehot = tf.one_hot(self.chosen_goals, num_goals, dtype=tf.float32)
                    qvalues_for_chosen_goals = tf.reduce_sum(self.qvalues * goals_onehot, axis=1)
                    td_error = tf.square(self.target_q_values - qvalues_for_chosen_goals)
                    self.qvalue_loss = 0.5 * tf.reduce_mean(td_error)

                    params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'highlevel/%s'%scope)
                    ...
                    global_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'highlevel/global')
        ...
        with tf.variable_scope('lowlevel'):
            with tf.variable_scope(scope):
                ...
                for i in range(len(conv_layers)):
                    ...
                    if pool_layers[i] is not None:
                        result = slim.max_pool2d(inputs=result,
                                                 ...
                                                 scope='pool_%d' % i)
                flatten = slim.flatten(result)
                # embed = tf.concat([flatten, self.goal], 1)
                hidden_embed = slim.fully_connected(inputs=flatten,
                                                    num_outputs=100,
                                                    activation_fn=None,
                                                    weights_initializer=tf.contrib.layers.xavier_initializer(),
                ...
                if scope != 'global':
                    ...
                    params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'lowlevel/%s'%scope)
                    gradients = tf.gradients(self.qvalue_loss, params)
                    norm_gradients, _ = tf.clip_by_global_norm(gradients, 40.0)

                    trainer = tf.train.RMSPropOptimizer(learning_rate=self.lr)
                    global_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'lowlevel/global')
                    self.update = trainer.apply_gradients(zip(norm_gradients, global_params))
