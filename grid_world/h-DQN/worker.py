...
class Worker():
    def __init__(self,
        ...
        self.update_target_ops = update_multiple_target_graphs(
            from_scopes=['highlevel/global/main', 'lowlevel/global/main'],
    ...
    def _train_highlevel(self,
               sess):
        # replay_buffer: [0:state, 1:action, 2:reward, 3:done, 4:new_state, 5:target, 6:valid_targets]
        with sess.as_default():
            batch = self.highlevel_replay_buffer.sample(FLAGS.batch_size)
            next_q_prime = sess.run(self.highlevel_network_target.qvalues,
                                    feed_dict={self.highlevel_network_target.state: np.stack(batch[:, 4]),
                                               self.highlevel_network_target.goal: np.stack(batch[:, 5])})
            next_q = sess.run(self.highlevel_network_main.qvalues,
                              feed_dict={self.highlevel_network_main.state: np.stack(batch[:, 4]),
                                         self.highlevel_network_main.goal: np.stack(batch[:, 5])})
            next_valid_targets = batch[:, 6]
            ...
            qvalue_loss, _ = sess.run([self.highlevel_network_main.qvalue_loss,
                                      ...
                                      feed_dict={self.highlevel_network_main.state: np.stack(batch[:, 0]),
                                                 self.highlevel_network_main.goal: np.stack(batch[:, 5]),
                                                 self.highlevel_network_main.chosen_goals: batch[:, 1],
    ...
    def _run_training_episode(self,
        ...
        highlevel_state = env.get_around_state(FLAGS.window_size / 2, state_type='map_all_goals')
        ...
        for _ in range(FLAGS.max_episode_steps):
            ...
            if episode_steps == 0 or sub_done or termination or lowlevel_steps == FLAGS.max_lowlevel_episode_steps:
                ...
                if np.random.rand() < epsilon:
                    ...
                    highlevel_qvaules = sess.run(self.highlevel_network_main.qvalues,
                                                 feed_dict={self.highlevel_network_main.state:[np.stack(highlevel_state)],
                                                            self.highlevel_network_main.goal:[target_input]})[0]
                    sub_target = valid_targets[np.argmax(highlevel_qvaules[valid_targets])]
            ...
            if not sub_done and sub_target in valid_targets:
                if np.random.rand() < ep_end or sub_target == FLAGS.num_goals: #epsilon:
            ...
            new_highlevel_state = highlevel_state[1:] + [env.get_around_state(FLAGS.window_size/2, state_type='map_all_goals')]
            ...
            if not testing:
                self.highlevel_replay_buffer.add(np.reshape(np.array([np.stack(highlevel_state),
                                                                      sub_target,
                                                                      reward,
                                                                      done,
                                                                      np.stack(new_highlevel_state),
                                                                      target_input,
                                                                      valid_targets]), [1, -1]))
        ...
        if testing:
            return disc_cumu_rewards, action_steps, min_step, done, states_buffer, goals_buffer, actions_buffer

        hl = np.mean(highlevel_losses) if len(highlevel_losses) != 0 else 0
        ll = np.mean(lowlevel_losses) if len(lowlevel_losses) != 0 else 0
        return disc_cumu_rewards, action_steps, min_step, done, hl, ll
    ...
    def test(self):
        with tf.Session() as sess:
            ...
            while self.episode_count < FLAGS.max_episodes:
                ...
                scene = '%s/valid/map_%04d' % (FLAGS.env_dir, 9)
                target = 12
                start_postions = [4, 5]
