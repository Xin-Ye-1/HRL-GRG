...
flags.DEFINE_integer('highlevel_num_channels', 17, 'The number of vision channels.')
...
def set_up():
    ...
    with graph.as_default():
        ...
        for i in range(FLAGS.num_threads):
            ...
            # relation_graph = Dirichlet_Multimodel_Graph(num_nodes=FLAGS.num_goals,
            #                                             num_categories=FLAGS.max_lowlevel_episode_steps+1,
            #                                             gamma=FLAGS.gamma)
            # graph_params = np.zeros((FLAGS.num_goals, FLAGS.num_goals, FLAGS.max_lowlevel_episode_steps + 1))
            # graph_params[:, :, -1] = 1
